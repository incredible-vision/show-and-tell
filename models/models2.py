...
from jisu_module import Discriminator
...
class ShowAttendTellModel_G(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.lstm = nn.LSTM(self.embed_size, self.hidden_size, self.num_layers, batch_first=False)
        self.classifier = nn.Linear(self.hidden_size, self.vocab_size)
        self.argMax = argMax_gumble()

        self.max_length = 20

        annFile = '/home/jisu/Desktop/Data/COCO/annotations_captions/captions_val2014.json'
    ...
    def forward(self, images, captions, maxlen=None):
        ...
        for t in range(seqlen):
            ...
            hidden, state = self.lstm(xt.unsqueeze(0), state)
            output = self.classifier(hidden.squeeze(0))
            output, output_idx = self.argMax(output)
            outputs.append(output)
            ...
            word_emb = self.embedding(output_idx).squeeze(1)
            outputs_embedding.append(word_emb)
        ...
        return outputs, outputs_embedding[:self.max_length], seqlen, xt
    ...
    def sample_multinomial(self, images, max_length=20):
        ...
        return outputs_multinomial, outputs_embedding, xt
