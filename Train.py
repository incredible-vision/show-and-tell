...
class Trainer_GAN(object):
    def __init__(self, opt, trainloader, validloader):
        ...
        self.G_optimizer = optim.Adam(G_parameters, lr=opt.learning_rate * 100)
    ...
    def train(self):
        ...
        for epoch in range(loaded_epoch + 1, 1 + self.opt.max_epochs):
            ...
            for iter, (images, captions, lengths, cocoids) in enumerate(self.trainloader):
                ...
                f_sentences, f_sentences_word_emb, embed_img = self.model_G.sample_multinomial(images) # ([128,1] x20)/ ([128,512]x20)
                embed_img = embed_img.detach()
                r_sentences, r_sentences_word_emb = self.model_G.gt_sentences(captions) # ([128,1] x20)/ ([128,512]x20)
                # r_sentences_onehot = [torch.LongTensor(batch_size, self.opt.vocab_size).zero_().cuda() for _ in range(20)]
                # r_sentences_onehot = [torch.LongTensor(batch_size, self.opt.vocab_size).zero_().cuda().scatter_(1, r_sentences[i].data, 1) for i in range(20)]
                # print r_sentences_onehot[0].size()
                f_label, r_label = Variable(torch.LongTensor(f_sentences[0].size(0)).cuda()), Variable(torch.LongTensor(f_sentences[0].size(0)).cuda())
                f_label.data.resize_(f_sentences[0].size(0)).fill_(0)
                r_label.data.resize_(f_sentences[0].size(0)).fill_(1)
                f_sentences_word_emb = [t.detach() for t in f_sentences_word_emb]
                f_D_output_img, f_D_output_MLE = self.model_D(f_sentences_word_emb, embed_img)
                f_D_output_MLE = list(f_D_output_MLE)
                f_error_MLE = [self.criterion_1(f_D_output_MLE[c], r_sentences[c][:, 0]) for c in range(20)]
                f_error_MLE = torch.cat(f_error_MLE).sum()
                f_error = self.criterion(f_D_output_img, f_label) + f_error_MLE * 0.001
                f_error.backward()

                r_D_output_img, r_D_output_MLE = self.model_D(r_sentences_word_emb, embed_img)
                r_error = self.criterion(r_D_output_img, r_label)
                r_error.backward()
                ...
                f_outputs, f_outputs_emb, _, embed_img = self.model_G(images, captions)

                f_label = Variable(torch.LongTensor(f_sentences[0].size(0)).cuda())
                f_label.data.resize_(f_sentences[0].size(0)).fill_(1)

                f_D_output_img, f_D_output_MLE = self.model_D(f_outputs_emb, embed_img)
                f_error_MLE = [self.criterion_1(f_D_output_MLE[c], r_sentences[c][:, 0]) for c in range(20)]
                f_error_MLE = torch.cat(f_error_MLE).sum()
                G_error = self.criterion(f_D_output_img, f_label) + f_error_MLE * 0.001
                ...
                if iter % self.opt.log_step == 0:
                    print('[%d/%d][%d/%d] Loss_D: %.4f Loss_G: %.4f'
                          % (epoch, self.opt.max_epochs, iter, len(self.trainloader), D_error.data[0], G_error.data[0]))
                    train_loss_history[total_iteration] = {'loss': G_error.data[0]}
                    self.train_loss_win = visualize_loss(self.train_loss_win, train_loss_history, 'train_loss', 'loss')
                ...
                if (total_iteration % self.opt.save_checkpoint_every == 0):
                    ...
                    # Write the training loss summary
                    # loss_history[total_iteration] = loss.data[0].cpu().numpy()[0]
                    loss_history[total_iteration] = G_error.data[0]
                    lr_history[total_iteration] = self.opt.current_lr
